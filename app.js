...
const configs = {
  levo: {
    ...
    columns: [
      ...
      { key: "value", label: "Valor", type: "money", readonly: true },
      { key: "kmStart", label: "KM saída", type: "number" },
      { key: "kmEnd", label: "KM chegada", type: "number" },
      { key: "situation", label: "Situação", type: "select", options: ["CONCLUÍDO", "AUTORIZADOR", "PENDENTE"] },
      { key: "km", label: "KM", type: "number", readonly: true },
    ],
  },
  bomSabor: {
    title: "Bom Sabor",
...
function calculateLevo(record, rate = state.kmRate) {
  ...
  const kmEnd = Number(record.kmEnd || 0);
  const km = Math.max(0, kmEnd - kmStart);
  return { ...record, km, value: km * rate };
}
...
function updateKmRate(event) {
  state.kmRate = Number(event.target.value || 0);
  state.data.levo = state.data.levo.map((record) => calculateLevo(record));
  saveState();
  render();
}
...
function saveFromForm(event) {
  ...
  config.columns.forEach((column) => {
    ...
    record[column.key] = column.type === "number" || column.type === "money" ? Number(input.value || 0) : input.value.trim();
  });
...
function createInput(column) {
  ...
  input.type = column.type === "money" || column.type === "number" ? "number" : column.type;
...
function renderTableHead() {
  const cells = getConfig().columns
    .map((column) => `<th><button type="button" data-sort="${column.key}">${escapeHtml(column.label)}</button></th>`)
    .join("");
...
function renderSummary() {
  ...
  elements.averageValue.textContent = moneyFormatter.format(records.length ? total / records.length : 0);
  elements.levoTotalHeader.textContent = moneyFormatter.format(total);
}

function exportCsv() {
  const config = getConfig();
  const header = config.columns.map((column) => column.label);
  const rows = getSortedRecords().map((record) => config.columns.map((column) => displayValue(record, column, false)));
...
function shareWhatsApp() {
  ...
  const lines = records
    .slice(0, 8)
    .map((record) => `${formatDate(record.date)} - ${record[config.personField] || "-"} - ${moneyFormatter.format(Number(record[config.valueField] || 0))}`)
    .join("\n");
...
function buildReportLines() {
  ...
  const header = config.columns.map((column) => fitCell(column.label, widths[column.key])).join(" | ");
  const separator = config.columns.map((column) => "-".repeat(widths[column.key])).join("-+-");
...
function createPdf(lines) {
  ...
  pages.forEach((linesForPage) => {
    const content = ["BT", "/F1 7 Tf", "10 TL", "24 560 Td", ...linesForPage.map((line) => `(${pdfEscape(line)}) Tj T*`), "ET"].join("\n");
